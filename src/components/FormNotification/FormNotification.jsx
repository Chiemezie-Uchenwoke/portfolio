...
const FormNotification = ({title, message, onClose, isSuccess, isError}) => {
    return (
        <div className="fixed top-0 left-0 z-[70] bg-dark-bg/60 flex justify-center items-center w-full h-full">
            <div className={`bg-lite-gray w-[90%] max-w-100 relative z-[75] rounded-lg py-8 px-4 border ${isSuccess ? "border-green-600" : "border-black/30"} ${isError ? "border-red-400" : "border-black/30"} shadow-lg flex flex-col gap-4`}>
                ...
                <h2 className={`capitalize font-medium ${isSuccess ? "text-green-500" : ""} ${isError ? "text-red-400" : ""}`}> {title} </h2>
